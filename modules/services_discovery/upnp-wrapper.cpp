...
vlc::threads::mutex UpnpInstanceWrapper::s_lock_instance;
vlc::threads::mutex UpnpInstanceWrapper::s_lock_listeners;
...
std::string generateUDN()
{
    ...
    if( ipv4addr )
        vlc_hash_md5_Update( &hash, ipv4addr, strlen( ipv4addr ) );
    char ipv4port[7] = {};
    snprintf( ipv4port, 7, ":%hu\n", UpnpGetServerPort() );
    vlc_hash_md5_Update( &hash, ipv4port, strlen( ipv4port ) );

#ifdef UPNP_ENABLE_IPV6
    const char *ipv6addr = UpnpGetServerIp6Address();
    if( ipv6addr )
        vlc_hash_md5_Update( &hash, ipv6addr, strlen( ipv6addr ) );
    char ipv6port[7] = {};
    snprintf( ipv6port, 7, ":%hu\n", UpnpGetServerPort6() );
    vlc_hash_md5_Update( &hash, ipv6port, strlen( ipv6port ) );
#endif

    char hexhash[VLC_HASH_MD5_DIGEST_HEX_SIZE];
    vlc_hash_md5_Finish( &hash, &hexhash, VLC_HASH_MD5_DIGEST_HEX_SIZE );

    std::string udn( hexhash, VLC_HASH_MD5_DIGEST_HEX_SIZE );
    udn.insert( 20, "-" );
    udn.insert( 16, "-" );
    udn.insert( 12, "-" );
    udn.insert( 8, "-" );
